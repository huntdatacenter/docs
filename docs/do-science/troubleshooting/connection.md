...
We allow connection from known IP addresses only. This means that your VPN connection may be blocked if you connect from a (for us) unknown network outside Norway. We are happy to open for new connections. Look up your IP address (<MyIPAddress />) and request your VPN access list opening in our [service desk](/do-science/service-desk/#vpn-access-list).
