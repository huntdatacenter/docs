      ...
      <v-expansion-panels v-model="mainExpansionPanel" elevation="0">
        ...
        >
          ...
          <v-expansion-panel-text id="fetch-secrets" class="mt-2">
            ...
            <ol>
              ...
              <li>
                Unpack (extract) the file only with <a href="/do-science/tools/transfer/7z/#detail-2-macos" target="_blank">software that supports the 7-ZIP archive format</a>.
              </li>
              ...
              <v-card>
                ...
                <v-card-text class="pa-0">
                  <v-stepper-vertical v-model="vpnStepper" hide-actions :editable="false">
                    <v-stepper-vertical-item :complete="vpnStepper > 1" value="1" title="Install Tunnelblick">
                      <v-card class="mb-8 pr-4" elevation="0">
                        <v-card-text>
                          ...
                          <a href="https://tunnelblick.net/downloads.html#:~:text=GnuPG%20v2%20signature-,Stable,-Tunnelblick" target="_blank"
                    ...
                    <v-stepper-vertical-item :complete="vpnStepper > 2" value="2" title="Setup the VPN profile">
                      <v-card class="mb-8 pr-4" elevation="0">
                        <v-card-text>
                    ...
                    <v-stepper-vertical-item :complete="vpnStepper > 3" value="3" title="Connect to the VPN">
                      <v-card class="mb-8 pr-4" elevation="0">
                        <v-card-text>
                          <ol>
                            ...
                            <li>
                              ...
                              <CopyTextField :model-value="username" label="" prefix="" placeholder="Your link is missing access token" />
                            </li>
                            <li>
                    ...
                    <v-stepper-vertical-item :complete="vpnStepper > 4" value="4" title="Verify your VPN connection">
                      <v-card class="mb-8 pr-16" elevation="0">
                        <v-card-text>
                          <v-alert border="start" border-color="success" elevation="2" icon="mdi-chevron-right">
                            ...
                            <hr class="mt-1 mb-2" />
                            A small Tunnelblick window should state <b>"Connected"</b> in green letters with a timer that count the connection length. <br /><br />
                            You should also see it by hovering your mouse over Tunnelblick icon in
                            <a href="https://support.apple.com/en-ie/guide/mac-help/mchlp1446/mac" target="_blank">the menu bar</a>.
                          </v-alert>
                        </v-card-text>
                      </v-card>

                      <template v-slot:actions>
                        <v-btn color="success" class="mx-2 mb-1" @click="closeVPNDialog(1)">Finish</v-btn>
                    ...
                    <v-stepper-vertical-item value="5" title="Troubleshooting VPN" subtitle="Optional tips to try in case of issues">
                      <v-card class="mb-8 pr-4" elevation="0">
                        <v-card-text>
                          <br />
                          <h3 id="ovpn-permissions-privilege"><a href="#ovpn-permissions-privilege" class="header-anchor">#</a> You do not have permission to open the document</h3>
                          <div>
                            If you see one of these errors:

                            <div class="language- extra-class">
                              <pre class="language-text">
                                <code v-text="`You do not have permission to open the document &quot;${username}.ovpn&quot;.\nContact your computer or network administrator for assistance.`"></code>
                              </pre>
                            </div>
                            <div class="language- extra-class">
                              <pre class="language-text">
                                <code v-text="`Du har ikke tillatelse til å åpne dokumentet &#xAB;${username}.ovpn&#xBB;.\nBe maskinens eller nettverkets administrator om hjelp.`"></code>
                              </pre>
                            </div>
                            ...
                            <ul>
                              ...
                              <li>
                                Right click on <code>{{ username }}.ovpn</code> and choose <strong><code>Get Info</code></strong
                                >.
                                <img class="pa-2" alt="macos-permissions-step1" src="/img/macos/permissions/step1-get-info.png" style="max-width: 400px" />
                              </li>
                              <li>
                                Find <strong><code>Sharing & permissions</code></strong> section in the end:
                                <img class="pa-2" alt="macos-permissions-step2" src="/img/macos/permissions/step2-find-permissions.png" style="max-width: 400px" />
                              </li>
                              <li>
                                Change privilege from <strong><code>No Access</code></strong> to <strong><code>Read only</code></strong
                                >:
                                <img class="pa-2" alt="macos-permissions-step3" src="/img/macos/permissions/step3-change-readonly.png" style="max-width: 400px" />
                              </li>
                            </ul>
                          </div>
                          ...
                          <p>If you want to check whether your VPN connection is blocked by the firewall, install nmap using: <code>brew install nmap</code> and then run:</p>
                          ...
                          <v-alert border="start" border-color="info" elevation="2" class="mt-6">
                            ...
                            If nothing works, please head over to our main <a href="/do-science/troubleshooting/connection/#vpn" target="_blank">troubleshooting section</a> for
                            more information on how to troubleshoot connections.
                          </v-alert>
                        </v-card-text>
                      </v-card>
        ...
        <v-expansion-panel :disabled="!filterGuidesByType || ['new_user', 'new_computer', 'new_lab', 'reissue_all', 'reissue_all'].includes(filterGuidesByType) ? false : true">
          ...
          <v-expansion-panel-text id="passwordless-access" class="mt-2">
            ...
            <v-col cols="12">
              {{ getNextItem(passLessId, true) }} Open new Terminal window and generate ssh key. If command reports that id_rsa key already exists, to avoid overwriting your
              existing keys press <code>n</code> and skip to next step.
              <CopyTextField
            ...
            <v-col cols="12">
              ...
              <CopyTextField :model-value="`eval &quot;$(ssh-agent -s)&quot;`" class="my-2" label="" prefix="~" placeholder="Your link is missing access token" />
            </v-col>

            <v-col cols="12">
              {{ getNextItem(passLessId) }} Add your public key to the ssh agent.
              <CopyTextField :model-value="`ssh-add --apple-use-keychain ~/.ssh/id_rsa`" class="my-2" label="" prefix="~" placeholder="Your link is missing access token" />
            </v-col>
            <v-col cols="12">
        ...
        <v-expansion-panel :disabled="!filterGuidesByType || ['new_user', 'new_computer', 'new_lab', 'lab_migration', 'reissue_all'].includes(filterGuidesByType) ? false : true">
          ...
          <v-expansion-panel-text id="ssh-config" class="mt-2">
            ...
            <v-col cols="12">
              ...
              <CopyTextField :model-value="`open -Wne ~/.ssh/config`" class="my-2" label="" prefix="~" placeholder="Your link is missing access token" />
            </v-col>
            ...
            <v-col v-else cols="12">
              ...
              <CopyTextArea :model-value="configText" label="SSH Config file" placeholder="Your link is missing access token" rows="11" />
            </v-col>
            <v-col v-if="['lab_migration'].includes(filterGuidesByType)" cols="12">
              {{ getNextItem(sshConfId) }} Remove old fingerprint.
              <CopyTextField :model-value="`ssh-keygen -R ${labName}`" label="" prefix="~" placeholder="Your link is missing access token" />
            </v-col>
            <v-col cols="12">
        ...
        <v-expansion-panel :disabled="!filterGuidesByType || ['new_user', 'new_computer', 'new_lab', 'lab_migration', 'reissue_all'].includes(filterGuidesByType) ? false : true">
          ...
          <v-expansion-panel-text id="hosts-file" class="mt-2">
            ...
            <br /><br />
            <v-col cols="12">
              {{ getNextItem(hostsFileId, true) }} On your local computer, open your /etc/hosts file in your preferred text editor. You will be asked for password that you use to
              login into your MacOS account. <br /><br />
              <div class="pl-8">
                Use this command if you prefer graphical <strong>Text editor</strong> app, and remeber to close the editor for the change to come into effect:
                <CopyTextField :model-value="`EDITOR='open -Wne' sudo -e /etc/hosts`" class="my-2" label="" prefix="$" placeholder="" />
            ...
            <v-col v-if="['lab_migration'].includes(filterGuidesByType)" cols="12">
              ...
              <CopyTextField :model-value="fqdn" class="my-2" label="" prefix="" placeholder="Your link is missing access token" />
            </v-col>
            <v-col v-if="['lab_migration'].includes(filterGuidesByType)" cols="12">
              {{ getNextItem(hostsFileId) }} Add (append) the new <strong>hosts record</strong> below to the text file:
              <br />
              <div class="pl-8">
                <CopyTextField :model-value="hostsWorkbench" class="my-2" label="" prefix="" placeholder="Your link is missing access token" />
              ...
              <v-card elevation="0">
                <v-card-title class="pa-0">
                  <v-toolbar color="#00509e" theme="dark" flat>
                    ...
                    <v-spacer />
                    <template v-slot:append>
                      <v-btn icon="mdi-close" @click="closeWorkbenchDialog()" />
                    </template>
                  </v-toolbar>
                </v-card-title>
                ...
                <v-card-text class="pa-0">
                  <v-stepper-vertical v-model="workbenchStepper">
                      ...
                      <v-card class="mb-8 pr-4" elevation="0">
                        <v-card-text>
                          <v-alert border="start" border-color="warning" elevation="2" class="mb-4">
                            ...
                            <hr class="mt-1 mb-2" />
                            If you do not see Profiles section in your System settings make sure to ask IT department in your organization for assistance.
                          </v-alert>
                          ...
                          <ol>
                            ...
                            <li>
                              Open section <code style="font-weight: bold">General</code>, scroll to the bottom of the section and select subsection
                              <code style="font-weight: bold">Device management</code>.
                              <br />
                            ...
                            <li>
                              Now quit your internet browser <code style="font-weight: bold">CMD + Q</code> (we recommend
                            ...
                            <li>
                              When you open your HUNT Workbench for the first time you will be asked for your local macOS password. This allows the browser to access your client
                              certificate stored in your local Keychain. <br />
                              After filling in the password, confirm by clicking <code style="font-weight: bold">Always allow</code> /
                    ...
                    <v-stepper-vertical-item :complete="workbenchStepper > 3" value="3" title="Login to Workbench">
                        ...
                        <v-card-text>
                          ...
                          <ol>
                            ...
                            <li class="mb-2">
                              ...
                              <CopyTextField :model-value="username" class="my-2" label="Username" prefix="" placeholder="Your link is missing access token" />
                      ...
                      <template v-slot:actions>
                        <v-btn color="success" class="mx-2 mb-1" @click="closeWorkbenchDialog(1)">Finish</v-btn>
                    ...
                    <v-stepper-vertical-item value="4" title="Troubleshooting">
                      <v-card class="mb-8 pr-4 ml-0 pl-0" elevation="0">
                        <v-card-text>
                          ...
                          <details class="my-2">
                            <summary style="cursor: pointer"><strong>You do not have permission to open the document</strong></summary>
                            <div class="pl-4 pr-16 py-2">
                              If you see one of these errors:

                              <div class="language- extra-class">
                                <pre class="language-text">
                                  <code v-text="`You do not have permission to open the document &quot;${labName}-${username}.mobileconfig&quot;.\nContact your computer or network administrator for assistance.`"></code>
                                </pre>
                              </div>
                              <div class="language- extra-class">
                                <pre class="language-text">
                              ...
                              <ul>
                                <!-- docs/public/img/macos/permissions/step1-get-info.png -->
                                <li>
                                  Right click on <code>{{ labName }}-{{ username }}.mobileconfig</code> and choose <strong><code>Get Info</code></strong
                                  >.
                                  <img class="pa-2" alt="macos-permissions-step1" src="/img/macos/permissions/step1-get-info.png" style="max-width: 400px" />
                                </li>
                                <li>
                          ...
                          <details class="my-2">
                            ...
                            <div class="pl-4 pr-16 py-2">
                              Don't worry. Request a <a href="/do-science/service-desk/#ssh-passphrase-reset" target="_blank">reset of SSH passphrase</a> in our "do-science"
                              Service desk.
                            </div>
                          </details>
                      ...
                      <template v-slot:actions>
                        ...
                        <v-btn color="primary" variant="text" class="mx-2 mb-1" @click="closeWorkbenchDialog(1)">Close</v-btn>
                      </template>
                    </v-stepper-vertical-item>
                  </v-stepper-vertical>
                </v-card-text>
        ...
        <v-expansion-panel>
          ...
          <v-expansion-panel-text id="copypubkey" class="mt-2">
            ...
            <v-col cols="10">
              ...
              <CopyTextField :model-value="`pbcopy < ~/.ssh/id_rsa.pub`" label="" prefix="~" placeholder="Your link is missing access token" />
            </v-col>
        ...
        <v-expansion-panel>
          ...
          <v-expansion-panel-text id="troubleshooting" class="mt-2">
            <v-col cols="12">
              ...
              Make sure to allow Keka access to your home folder in <b>File Access</b> section of Keka. <img alt="tunnelblick-login" src="/img/7-zip/keka1.png" /> <br />
            </v-col>
          </v-expansion-panel-text>
        </v-expansion-panel>
      </v-expansion-panels>
