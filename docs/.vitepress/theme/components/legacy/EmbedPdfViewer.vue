...
const zoom = ref(1)
const defaultScaleDelta = ref(1.1)
const pageWidth = ref(null)
const minScale = ref(0.25)
const maxScale = ref(10.0)
const default_scale = ref(1)
const initialPdfWidth = ref(null)
const pdf = ref(null)
...
const render = doc => {
  ...
  showPdf.value = true
  nextTick(() => {
    initialPdfWidth.value = setPdfInitWidth()
  })
}
...
const nextPage = () => {
  if (page.value) {
    page.value = page.value < props.pages ? page.value + 1 : props.pages
  } else {
    page.value = props.pages > 1 ? 2 : 1
  }
}

const setPdfInitWidth = () => {
  const pdfElement = document.getElementById("viewer")
  if (pdfElement) {
    initialPdfWidth.value = pdfElement.width ? pdfElement.width : pdfElement.offsetWidth
  }
}

const scalePdfWidth = zoomValue => {
  if (!initialPdfWidth.value) {
    setPdfInitWidth()
  }
  const zoomWidth = Math.ceil(zoomValue * initialPdfWidth.value)
  pageWidth.value = zoomWidth
  console.log(`Page width on zoom: ${zoomWidth}`)
}

const zoomIn = () => {
  let newScale = zoom.value
  newScale = (newScale * defaultScaleDelta.value).toFixed(2)
  newScale = Math.ceil(newScale * 10) / 10
  newScale = Math.min(maxScale.value, newScale)
  zoom.value = newScale
  scalePdfWidth(zoom.value)
}
...
const zoomOut = () => {
  ...
  newScale = Math.max(minScale.value, newScale)
  zoom.value = newScale
  scalePdfWidth(zoom.value)
}
...
<template>
  <div id="pdf-viewer">
    ...
    <footer>
      <button class="toolbarButton pageUp ripple" title="Previous Page" id="previous" @click="previousPage"></button>
      <button class="toolbarButton pageDown ripple" title="Next Page" id="next" @click="nextPage"></button>
      ...
      <button class="toolbarButton zoomOut" title="Zoom Out" id="zoomOut" @click="zoomOut"></button>
      <button class="toolbarButton zoomIn" title="Zoom In" id="zoomIn" @click="zoomIn"></button>
    </footer>
  </div>
</template>
