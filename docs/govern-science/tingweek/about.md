---
title: Tingweek overview
category: Tingweek
permalink: /govern-science/tingweek/overview
outline: 1
description: This gives an overview of the HUNT Cloud Tingweek.
---

# About Tingweeks

**We appoint time three times per year to meet for information, feedback and discussions on how to develop the services in HUNT Cloud to better elevate science. We call these gatherings Tings.**

We arrange a layered set of Tings for distinct groups in our community. The meetings are spread out over one week in the winter, spring and autumn. We call these weeks Tingweeks.

::: tip Next Tingweek
[Click here](/govern-science/tingweek/dates) to see the dates for upcoming Tingweeks.
:::

[[toc]]


## The different Tings

We appoint time for three different Tings during the Tingweek to elevate voices and collective knowledge from distinct groups within our HUNT Cloud community. All users and administrators in HUNT Cloud are welcome to participate in all Tings.

### Labting

This is our allting were we focuses on **user experiences inside labs**. All active lab users, lab leaders and lab coordinators are invited for information, feedback and discussions on all aspects of existing, planned and future services. [Read more about the Labting](/do-science/community/labting) in our do science section.

### Admin ting

This is the Ting where we focuses on **administrative aspects** in labs and data spaces. All data space administrators, lab leaders and lab coordinators are invited for information, feedback and discussion on administration of data, technologies and training. The Admin ting does also cover financial aspects related to prices, invoices and services specifications. Ting resolutions are also announced in this Ting.

### Data controller ting

This is the Ting where we initiate **cross-organizational dialogue on privacy, security and ethics** in HUNT Cloud. All privacy and security contacts, and data space compliance administrators, are invited. 


## Schedule

#### Dates

::: tip Ting dates
[Click here](/govern-science/tingweek/dates) to see past and future Ting dates.
:::


#### Times

Our Tingweeks are scheduled over one week in winter, spring and autumn. 

We aim to schedule our winter Ting mid February in the week before Norway's winter holidays (vinterferie). The spring Ting is scheduled in late May / early June. The autumn Ting in scheduled in the second half of October, usually the week after Norway's autumn holiday (høstferie).

Individual Tings are scheduled over the week on fixed time slots. The current schedule is as follows: Labting is held on Tuesdays 2-4 pm CET, Admin ting on Wednesdays 9-11 am CET and Data controller ting on Wednesdays 1-3 pm CET.


## Agenda

All users are welcome to propose agenda items for the next meetings. [Click here](/govern-science/tingweek/faq) to see past agendas.


## Resolutions

We document applied and intended development changes to our services in the form of resolutions. Resolutions from previous Tingweeks are available in our resolutions page. All administrators are welcome to propose resolutions for the next meeting. 

::: tip Resolutions
[Click here](/govern-science/tingweek/resolutions) for past resolutions.
:::

Resolutions consist of two parts: (i) the introductory sentence that states the context of the resolution, and (ii) one to several operational clauses that describe the recommended action. 


## Frequently asked questions

::: tip FAQ
[Click here](/govern-science/tingweek/faq) to read our Tingweek FAQ.
:::

## Historical development

We aim to continuously evolve the Tingweeks a long with evolving scientific and administrative needs: 

- Tingweek #18: The Financial ting and Admin ting was merged. Lagtun resolutions was renamed Ting resolutions and from now promoted directly by HUNT Cloud.
- Tingweek #14: The Coordinator ting changed name to Admin ting to focus on administrative aspects. Lagtun changed name to Financial ting.
- Tingweek #5: The original Coordinator ting changed name to Data space ting to reflect contractual roles in the new services model.
- Tingweek #1-6: We published a Ting report for the first Tingweeks: [Ting report #6 (2022-05)](https://assets.hdc.ntnu.no/assets/tingweek/hunt-cloud-tingweek6-report.pdf), [Ting report #4 (2021-10)](https://assets.hdc.ntnu.no/assets/tingweek/hunt-cloud-tingweek4-report.pdf), [Ting report #3 (2021-05)](https://assets.hdc.ntnu.no/assets/tingweek/hunt-cloud-tingweek3-report.pdf), [Ting report #2 (2021-02)](https://assets.hdc.ntnu.no/assets/tingweek/hunt-cloud-tingweek2-report.pdf), [Ting report #1 (2020-10)](https://assets.hdc.ntnu.no/assets/tingweek/hunt-cloud-tingweek1-report.pdf).
- Tingweek #1: The Tingweek was introduced in 2020 at the time we introduced our price model.
